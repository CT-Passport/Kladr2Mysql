...
/*
 * Extra tasks:
 * ALTER TABLE `addrob` DROP `enddate`;
 * ALTER TABLE `addrob` DROP `actstatus`;
 * ALTER TABLE `addrob` DROP `updatedate`;
 * ALTER TABLE `addrob` DROP `ctarcode`;
 * ALTER TABLE `addrob` DROP `extrcode`;
 * ALTER TABLE `addrob` DROP `sextcode`;
 * ALTER TABLE `addrob` DROP `normdoc`;
 * ALTER TABLE `addrob` DROP `startdate`;
 * update `addrob` set parentguid = null where parentguid="";
 * ALTER TABLE `addrob` DROP INDEX `aoid`;
 * ALTER TABLE `addrob` ADD INDEX(`parentguid`);
 * OPTIMIZE TABLE `addrob`;
 * ANALYZE TABLE `addrob`;
 **/
...
class fias
{
    ...
    public $debug = false;
    public $indexTable = [
        'actstat' => ['PRIMARY KEY' => 'actstatid'],
        'addrob' => ['PRIMARY KEY' => 'aoid',
    ...
    public function import()
    {
        ...
        foreach ($files as $file) {
            if ($file != '.' && $file != '..' && strtolower(pathinfo($file, PATHINFO_EXTENSION)) == 'dbf') {
                ...
                $this->tableName = preg_replace('/\d/', '', $this->tableName);
                ...
                while ($record = $this->nextRecord()) {
                    ...
                    foreach ($this->columns as $column) {
                        switch ($column['type']) {
                            ...
                            case self::DATE:
                            case self::CHAR:
                                $val[] = "'" . $this->getString($column['name']) . "'";
                                break;
                            case self::NUMERIC:
                                $val[] = (int)$this->getNum($column['name']);
                                break;
                            default:
                                $val[] = "'" . $this->getObject($column) . "'";
                                break;
                        }
                    }
    ...
    */
    ...
    protected function createTable()
    {
        ...
        foreach ($this->columns as $column) {
            ...
            if ($type) {
                if (isset($this->indexTable[$baseName]['PRIMARY KEY']) && $this->indexTable[$baseName]['PRIMARY KEY'] == $column['name']) {
                    $types[] = '' . $column['name'] . ' ' . $type . ' NOT NULL DEFAULT 1';
                } else {
        ...
        //$tableName = preg_replace("/[^A-Za-z0-9 ]/", '', $this->tableName);
    ...
    public function getObject($column)
    {
        switch ($column['type']) {
            case self::CHAR:
                return $this->getString($column['name']);
            case self::DOUBLE:
                return $this->getDouble($column['name']);
            case self::DATE:
                return $this->getDate($column['name']);
            case self::DATETIME:
                return $this->getDateTime($column['name']);
            case self::FLOATING:
                return $this->getFloat($column['name']);
            case self::LOGICAL:
                return $this->getBoolean($column['name']);
            case self::NUMERIC:
                return $this->getNum($column['name']);
            case self::INDEX:
                return $this->getIndex($column['name'], $column['length']);
            case self::IGNORE_0:
                return false;
        }
    ...
    public function getNum($columnName)
    {
        ...
        if ($column['type'] == self::NUMERIC && ($column['decimalCount'] > 0 || $column['length'] > 9)) {
            return doubleval($s);
        } else {
            return intval($s);
        }
    ...
    {
        $query = 'INSERT IGNORE INTO `' . $table . '` (`' . implode('`,`', $col) . '`) VALUES ' . implode(',', $values);
        //die;
        $query = str_replace(chr(0), '', $query);
